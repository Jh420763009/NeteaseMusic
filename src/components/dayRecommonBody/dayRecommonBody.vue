...
<div id="play-record-body">
	...
	<ul>
		...
		>
			...
			<div class="music-msg">
				<p>{{ item.name }}</p>
				<p>{{ item.artists | getSinger }} - {{ item.album.name }}</p>
			</div>
...
export default {
	...
	methods:{
		...
		toplay(idx){			
			...
			//存储当前歌单的id
			this.$store.commit('saveCurrentId','dayRecommon');
			//存储当前播放得index
	...
	mounted(){
		...
		if(waslogin){
			apiGet.recommendSong.to('/recommend/songs')
			.then((res)=>{
